...
def main(args):
	...
	x, y, z, _, d = nib.load(str(Path(phase_path_list[0]))).get_fdata().shape
	phase_data_list = np.zeros((x, y, z, args.number))

	phase_example = str(Path(phase_path_list[0]))
	for i in range(args.number):
		temp_data = nib.load(str(Path(phase_path_list[i]))).get_fdata()
		phase_data_list[:, :, :, i] = temp_data[:, :, :, 0, 0] / (args.tesla * gamma)  # change the slicing [:, :, :, 0, d], d=0, 1, 2 ...
