...
test_path = '/home/kuowei/Desktop/MICCAI_result/multiple_all/two/'
...
for i in range(len(subject_list)):
	...
	for j in range(j_len):
		...
		test_img = nib.load(test_path + subject_list[i] + '_ori1' + '_LBVSMV_pred1_' + str(j+1) + '_qsm.nii.gz')
		#test_img = nib.load(test_path + subject_list[i] + '_ndi3_echo_Avg.nii.gz')
		test_data = test_img.get_fdata()#[:, :, :, np.newaxis]

		gt_data = np.load(gt_path + subject_list[i] + '/' + subject_list[i] + '_cosmos.npy')

		mask = np.load(mask_path + subject_list[i] + '/ori1' + '/' +  subject_list[i] + '_ori1' + '_mask.npy')[:,:,:,np.newaxis]
