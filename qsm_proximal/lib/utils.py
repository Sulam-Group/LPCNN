...
from lib.model.pesp.pesp import PESP

from lib.loss.fwl1.fwl1 import fwl1
from lib.loss.l1ssim.l1ssim import l1ssim

def prepareDataset(args, root_dir, data_aug=None, normalize=False):
	
	if args.dataset.lower() == 'qsm':

		dataset_path = root_dir + 'qsm_dataset/qsm_B_r/mix5_data/partition/partition_data_list4/'
		
		train_dataset = QsmDataset(dataset_path, split='train', tesla=args.tesla, is_norm=normalize)
		val_dataset = QsmDataset(dataset_path, split='validate', tesla=args.tesla, is_norm=normalize)
...
def loadData(args, root_dir, prediction_data, normalize=False):
	...
	if args.dataset == 'qsm':
		...
		if case == 'whole':
			dataset_path = root_dir + 'qsm_dataset/qsm_B_r/mix5_data/whole/data_list1/'
		else:
			dataset_path = root_dir + 'qsm_dataset/qsm_B_r/mix5_data/partition/partition_data_list1/'

		if prediction_set == 'train':
			ext_data = QsmDataset(dataset_path, split='train', tesla=args.tesla, is_norm=normalize)
		elif prediction_set == 'val':
			ext_data = QsmDataset(dataset_path, split='validate', tesla=args.tesla, is_norm=normalize)
		elif prediction_set == 'test':
			ext_data = QsmDataset(dataset_path, split='test', tesla=args.tesla, is_norm=normalize)
...
def chooseModel(args, root_dir):
	...
	elif args.model_arch.lower() == 'pesp':
		model = PESP(root_dir + 'train_gt_mean.npy', root_dir + 'train_gt_std.npy')
	else:
...
def chooseLoss(args, option=0):
	...
	if args.model_arch.lower() == 'vdsr':
		...
		loss_fn = fwl1
	elif args.model_arch.lower() == 'wpdnn' and option == 2:
		loss_fn = l1ssim
		...
		loss_fn = fwl1
	elif args.model_arch.lower() == 'pudnn' and option == 2:
		loss_fn = l1ssim
	elif args.model_arch.lower() == 'pesp' and option == 0:
		loss_fn = nn.L1Loss()
	elif args.model_arch.lower() == 'pesp' and option == 1:
		loss_fn = fwl1
