...
class PUDNN(nn.Module):
	...
	def forward(self, y, dk, mask, train=True):
		...
		for b_n in range(batch_size):
			...
			if train:
				...
				x_est[b_n, :, :, :, :] = F.pad(self.alpha * torch.irfft(dk_batch[-1] * torch.rfft(y[b_n, :, :, :, :dim3_batch[-1]], 3, normalized=True, onesided=False), 3, normalized=True, onesided=False), (0, z_dim-dim3_batch[-1], 0, 0, 0, 0))
			...
			if i == 0:
				...
				for b_n in range(batch_size):
					if train:
						...
						pn_x_pred[b_n, :, :, :, :] = den_x_pred[b_n, :, :, :, :] - F.pad(self.alpha * torch.irfft(dk_batch[b_n] * dk_batch[b_n] * torch.rfft(den_x_pred[b_n, :, :, :, :dim3_batch[-1]], 3, normalized=True, onesided=False), 3, normalized=True, onesided=False), (0, z_dim-dim3_batch[-1], 0, 0, 0, 0))
